...
function App() {
  ...
  const handleSave = (e) => {
    ...
    if (lastName === '') // Fixed this condition
      error += 'Last Name is required. ';

    if (parseInt(age) <= 0) // Parse age to number
    ...
    if (error === '') {
      ...
      dt.push(newObject);
      setData(dt);
      handleClear(); // Corrected function name
    } else {
      alert(error)
    }
  }
  ...
  const handleUpdate = () => {
    const index = data.findIndex((item) => item.id === id); // Use findIndex to get the index
    if (index !== -1) {
      const dt = [...data];
